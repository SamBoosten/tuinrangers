...
class El_Blog_Module extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		$merge_array['post_order_by'] = array(
			...
			'tab_slug'         => 'general',
			'toggle_slug'      => 'loop_query',
			'description'      => esc_html__( 'Here you can choose the order type of your posts.', 'divi-blog-extras' ),
			'computed_affects' => array(
				'__dbe_posts',
			),
		);
		$merge_array['include_current_taxonomy']    = array(
			'label'            => esc_html__( 'Include Current Taxonomy Posts', 'divi-blog-extras' ),
			'type'             => 'yes_no_button',
			'option_category'  => 'configuration',
			'options'          => array(
				'on'  => esc_html__( 'Yes', 'divi-blog-extras' ),
		...
		$merge_array['current_taxonomies_relation']    = array(
			...
			if ( ! empty( $post_type_taxonomies ) ) {
				...
				foreach ( $post_type_taxonomies as $taxonomy_key => $post_type_taxonomy ) {
					if ( ! in_array( $taxonomy_key, $filtered_taxonomies, true ) ) {
						...
						$show_if        = array( 'include_current_taxonomy' => 'off' );
		...
		return array_merge(
			...
			array(
				...
				'masonry_columns'             	=> array(
					...
					'tab_slug'         => 'general',
					'toggle_slug'      => 'main_content',
					'description'      => esc_html__( 'Here you can select the number of columns for the Masonry Grid layout.', 'divi-blog-extras' ),
					'computed_affects' => array(
						'__dbe_posts',
					),
				),
	...
	public static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		if ( $query->have_posts() ) {
			...
			if ( ! $is_search && ! $is_archive && ! $is_slider && 'off' === $include_current_taxonomy && 'on' === $use_category_filterable_blog && 'post' === $query_args['post_type'] ) {
						...
						$output .= '<div class="el-dbe-filterable-categories" data-hamburger-filter="'. esc_attr( $use_hamburger_category_filter ) .'">';
							if ( 'on' === $use_hamburger_category_filter ) {
								$output .= '<div class="el-dbe-filterable-mobile-categories">';
									$output .= '<span class="el-dbe-mobile-active-category">' . esc_html( $all_posts_text ) . '</span>';
									$output .= '<span class="el-dbe-category-mobile-menu"></span>';
								$output .= '</div>';
							}
							$output .= '<ul class="el-dbe-post-categories">';
							if ( 'on' === $show_all_posts_link ) {
								$output .= '<li><a href="#" class="el-dbe-active-category" data-category="-1">' . esc_html( $all_posts_text ) . '</a></li>';
							}
							foreach ( $post_terms as $post_term ) {
						    	$output .= '<li><a href="#" data-category="' . esc_attr( $post_term->term_id ) . '">' . esc_html( $post_term->name ) . '</a></li>';
							}
							$output .= '</ul>';
						$output .= '</div>';
					}
			...
			while ( $query->have_posts() ) {
				...
				} elseif ( file_exists( plugin_dir_path( __FILE__ ) . 'layouts/' . sanitize_file_name( $blog_layout ) . '.php' ) ) {
					include plugin_dir_path( __FILE__ ) . 'layouts/' . sanitize_file_name( $blog_layout ) . '.php';
				}
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( $query->have_posts() ) {
			...
			if ( ! is_search() && ! is_archive() && ! $is_slider && 'off' === $include_current_taxonomy && 'on' === $use_category_filterable_blog && 'post' === $args['post_type'] ) {
				...
				if ( empty( $post_categories ) || '0' == count( $post_categories ) || '1' < count( $post_categories ) ) {
					...
					if ( ! empty( $post_terms ) && ! is_wp_error( $post_terms ) && 1 < count( $post_terms ) ){
						$output .= '<div class="el-dbe-filterable-categories" data-hamburger-filter="'. esc_attr( $use_hamburger_category_filter ) .'">';
							...
							if ( 'on' === $show_all_posts_link ) {
								$output .= '<li><a href="' . esc_url( $current_url ) . '" class="el-dbe-active-category" data-term-id="-1">' . esc_html( $all_posts_text ) . '</a></li>';
							}
							foreach ( $post_terms as $post_term ) {
						    	$output .= '<li><a href="' . esc_url( $current_url ) . '" data-term-id="' . esc_attr( $post_term->term_id ) . '">' . esc_html( $post_term->name ) . '</a></li>';
							}
							$output .= '</ul>';
						$output .= '</div>';
					}
			...
			if ( 'on' === $show_load_more && ! $is_slider ) {
				add_filter( 'get_pagenum_link', array( 'El_Blog_Module', 'filter_pagination_url' ) );
				// Pagination.
				if ( 'on' === $pagination_type ) {
					// Load more Pagination.
					if ( $total > 1 ) {
				...
				remove_filter( 'get_pagenum_link', array( 'El_Blog_Module', 'filter_pagination_url' ) );
			}
