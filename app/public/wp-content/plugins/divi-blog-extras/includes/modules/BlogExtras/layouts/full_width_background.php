...
if ( '' !== $thumb && 'on' === $show_thumbnail ) {
	$back_thumb = el_get_post_thumbnail( $post_id, esc_html( $featured_image_size ), 'et_pb_post_main_image', false, true );
	$output    .= '<div class="post-media" style="background-image: url(' . esc_url( $back_thumb ) . ')">';
	$output    .= '<a href="' . esc_url( get_the_permalink( $post_id ) ) . '" target="' . $target . '" class="abs-url">' . esc_html( get_the_title( $post_id ) ) . '</a>';
	if ( 'on' === $use_overlay ) {
...
// Post Meta.
if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_categories || 'on' === $show_comments || 'on' === $show_read_time ) {
	...
	$output .= sprintf(
		'<p class="post-meta">%1$s %2$s %3$s %4$s %5$s %6$s %7$s %8$s %9$s</p>',
		(
			'on' === $show_author ?
			et_get_safe_localization(
				sprintf(
					'%1$s <span class="author vcard">%2$s</span>',
					esc_html__( 'By', 'divi-blog-extras' ),
		...
		(
			...
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
			'on' === $show_date ?
		...
		(
			( ( 'on' === $show_author || 'on' === $show_date ) && ( 'on' === $show_categories && ! empty( $post_term_list ) ) ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
			( 'on' === $show_categories && ! empty( $post_term_list ) ) ?
			et_core_intentionally_unescaped(
				sprintf(
					'<span class="post-categories">%s</span>',
					et_core_intentionally_unescaped( $post_term_list, 'html' )
				),
				'html'
			) :
			''
		...
		(
			( ( 'on' === $show_author || 'on' === $show_date || ( 'on' === $show_categories && ! empty( $post_term_list ) ) ) && 'on' === $show_comments ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
		...
		(
			( ( 'on' === $show_author || 'on' === $show_date || ( 'on' === $show_categories && ! empty( $post_term_list ) ) || 'on' === $show_comments ) && 'on' === $show_read_time ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
...
if ( 'on' === $show_content ) {
	...
	if ( has_excerpt() && 0 !== intval( $excerpt_length ) && '' !== trim( $post_object->post_excerpt ) ) {
	...
	if ( 'on' === $show_more ) {
		if ( 'on' === $use_read_more_button ) {
			...
			$more    = 'on' === $show_more ?
					(
						sprintf(
							'<p class="el-read-more-link"><a href="%1$s" target="%3$s" class="more-link">%2$s</a></p>',
