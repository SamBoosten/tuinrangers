<?php
/**
 * The Template for displaying Classic Layout
 *
 * This template can be overridden by copying it to yourtheme/divi-blog-extras/layouts/classic.php.
 *
...
if ( 'on' === $show_categories || 'on' === $show_date ) {
	...
	$output .= sprintf(
		'<p class="post-meta">%1$s %2$s %3$s</p>',
		(
		...
		(
			( 'on' === $show_categories && ! empty( $post_term_list ) && 'on' === $show_date ) ?
...
// Post Meta.
if ( 'on' === $show_author || 'on' === $show_social_icons || 'on' === $show_comments || 'on' === $show_read_time ) {
	$output .= sprintf(
		...
		(
			( 'on' === $show_author && 'on' === $show_social_icons ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
			'on' === $show_social_icons ?
			et_get_safe_localization(
				sprintf(
					'<span class="et-social-icons">
                        <span class="et-social-icon et-social-facebook">
                        	<a href="https://www.facebook.com/sharer.php?u=%1$s&t=%2$s" class="icon"></a>
                        </span>
                        <span class="et-social-icon et-social-twitter">
                        	<a href="https://twitter.com/share?text=%2$s&url=%1$s" class="icon"></a>
                        </span>
                    </span>',
		...
		(
			( ( 'on' === $show_author || 'on' === $show_social_icons ) && 'on' === $show_comments ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
		...
		(
			( ( 'on' === $show_author || 'on' === $show_social_icons || 'on' === $show_comments ) && 'on' === $show_read_time ) ?
			et_get_safe_localization( '<span class="divider"> | </span>' ) :
			''
		),
		(
