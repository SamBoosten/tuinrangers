...
if ( 'on' === $show_date ) {
	$post_date  = explode( '-', esc_html( get_the_date( 'M-Y-j', $post_id ) ) );
	...
	$output    .= sprintf(
		'<div class="post-date">
            <span class="published">
                <em class="month">%1$s</em>
                <em class="year">%2$s</em>
                <em class="day">%3$s</em>
            </span>
        </div>',
		esc_html( $post_month ),
...
// Post Meta.
if ( 'on' === $show_author || 'on' === $show_categories || 'on' === $show_comments || 'on' === $show_read_time ) {
	...
	$output .= sprintf(
		...
		(
			( 'on' === $show_author && ( 'on' === $show_categories && ! empty( $post_term_list ) ) ) ?
		...
		(
			( ( 'on' === $show_author || ( 'on' === $show_categories && ! empty( $post_term_list ) ) ) && 'on' === $show_comments ) ?
			et_core_intentionally_unescaped( ' | ', 'fixed_string' ) :
			''
		),
		(
		...
		(
			( ( 'on' === $show_author || ( 'on' === $show_categories && ! empty( $post_term_list ) ) || 'on' === $show_comments ) && 'on' === $show_read_time ) ?
			et_core_intentionally_unescaped( ' | ', 'fixed_string' ) :
			''
		),
		(
